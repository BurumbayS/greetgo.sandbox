...
public interface AuthDao {
  ...
  @Select("select cia_id from Person where accountName = #{accountName} " +
  ...
  @Select("select accountName from Person where cia_id = #{personId}")
  String accountNameByPersonId(@Param("personId") String personId);

  @Select("select * from Person where cia_id = #{personId}")
